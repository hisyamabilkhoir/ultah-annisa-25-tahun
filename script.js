...
var SCENES=[
  ...
  {id:'closing',lines:[
    {text:'Harapanmu untuk Annisa:',speed:45,pause:800,cls:'small'},
...
(function initGateStars(){
  ...
  function draw(){
    ...
    gStars.forEach(function(s){s.a+=s.s;if(s.a>1)s.s=-Math.abs(s.s);if(s.a<0)s.s=Math.abs(s.s);
...
function activateScene(id){
  ...
  if(id==='opening')startStarsCanvas();
...
async function handleInteraction(interact){
  ...
  if(interact.type==='input'){
    ...
    if(interact.id==='opening'){
      label.textContent='';
      await typeIntoElement(label,'Siapa namamu, penjelajah?',45);
    }
...
function initDecorations(){
  createSparkles();createFireflies();createOceanDots();createClouds();createHearts('hearts-final');createHearts('hearts-closing');
}
